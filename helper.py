...
class YOLOv8:
    ...
    def detect_objects(self, image):
        ...
        # Check if class_ids is not empty and contains the expected dimensions
        if self.class_ids.size > 0 and self.class_ids.ndim == 1:
            # Filter out all classes except 'person' (class_id 0)
            # person_indices = np.where(self.class_ids == 0)[0]
            # self.boxes = self.boxes[person_indices, :]
            # self.scores = self.scores[person_indices]
            # self.class_ids = self.class_ids[person_indices]

            if self.padding_percent is not None:
                self.boxes = self.add_padding(image)
                
            if self.filter_boxes_by_area is not None:
                self.boxes, self.scores, self.class_ids = self.filter_boxes_by_relative_area(image)
        else:
            # If class_ids is empty or not as expected, set all outputs to empty arrays
            self.boxes = np.array([])
